...
/**
 * 通过Request请求获取IP地址 <br>
 * 首先，我们获取 X-Forwarded-For 中第0位的IP地址，它就是在HTTP扩展协议中能表示真实的客户端IP。具体就像这样：
 * 
 * X-Forwarded-For: client, proxy1, proxy2，proxy…
 * 
 * 所以你应该知道为什么要取第0位了吧！ 如果 X-Forwarded-For 获取不到，就去获取X-Real-IP ，X-Real-IP
 * 获取不到，就依次获取Proxy-Client-IP 、WL-Proxy-Client-IP 、HTTP_CLIENT_IP 、
 * HTTP_X_FORWARDED_FOR 。最后获取不到才通过request.getRemoteAddr()获取IP，
 * 
